...
PubChem has labelled or computed internally [1]. 
  - This API is useful for querying information about a specific chemical 
  compound such as getting the standardized PubChem identifier (CID) for a given 
...
PubChem data usage is described by the [NCBI Website and Data Usage Policies](https://www.ncbi.nlm.nih.gov/home/about/policies/). Source: https://www.ncbi.nlm.nih.gov/home/about/policies/
...
```{r aspirin-cid, eval = online_examples}
mapCompound2CID("aspirin")
```

You can pass in a list of compound names to get the CIDs for all of them at 
once.

```{r multi-cid, eval = online_examples}
drugs <- c(
  "Aspirin",
  "Erlotinib",
  "Acadesine",
...
```{r multi-cid-first, eval = online_examples}
mapCompound2CID(drugs, first = TRUE)
```

In the case that a compound cannot be mapped, `NA` will be returned and a 
warning will be issued.

```{r missing-cid, eval = online_examples}
...
After deciding which properties to query, you can use the `mapCID2Properties` 
function to get the properties for a specific CID.

```{r single-cid, eval = online_examples}
...
```{r single-cid-annotations, eval = online_examples}
