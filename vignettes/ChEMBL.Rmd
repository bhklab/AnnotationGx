...
constants and ADMET data. The data is curated from primary scientific 
literature. The ChEMBL API allows for the data to be made available for 
retrieval in a programmatic fashion. We can use the API to query CHEMBL ID of a 
compound, retrieve all molecule mechanisms of action, query compound_record 
resource and molecule resource from the ChEMBL database.

## Licensing

ChEMBL is licensed under [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/). Source: https://www.ebi.ac.uk/chembl/
...
**
NOTE: This is a specialized function that queries the API for the *mechanism* 
resource only. To query other resources, please see the 
[Custom Queries](#custom-queries) section.
**



```{r single-query, eval = online_examples}
...
The ChEMBL API allows for a wide range of queries. We have specialized one 
function, but are open to incorporating more. Please open an issue at 
[bhklab/AnnotationGx](https://github.com/bhklab/AnnotationGx) with an idea of a 
specialized function that meets a use case. 
...
For example, the query for the example in 
[the above section](#chembl-mechanisms) would be:
...
`queryChemblAPI(resource, field, filter_type, value, format = "json")` function 
to query the ChEMBL API.

**NOTE: unlike the `getChemblMechanism()` function which returns a `data.table`, 
the `queryChemblAPI()` function returns the raw data unformatted**

```{r custom-query, eval = online_examples}
...
```{r resource-fields, eval = online_examples}
