...
are mapped per GET request. It also implements parallelized queries as well as
query throttling to ensure users do not exceed the 5 query per second, 400
query per minute limits imposed by PubChem. Both `getRequestPubChem` and
...
This can be used to diagnose HTTP errors using the `httr::headers(response)`
function.

Use of `queryPubChem` is mainly intended for developers, and beginners
...
```{r eval=FALSE}
headers <- getAllPubChemAnnotations()
...
`?getAllPubChemAnnotations` from the R console.

#### Common Use Cases

1. Get ChEMBL ID for a list of CIDs

```{r eval=FALSE}

(annotation <- headers[Heading %like% 'ChEMBL' & Type=="Compound", ]$Heading)

# get ChEMBL IDs for a list of CIDs 
CIDtoCHEMBL <- lapply(
  noDuplicatesNamesToCID$cids, 
  function(cid) as.data.table(getPubChemAnnotation(cid, annotationType=annotation)))

# merge results into a single data.table`
CIDtoCHEMBL <- merge(rbindlist(CIDtoCHEMBL, fill=TRUE), namesToCID, by.x = "cid", by.y = "cids", all.x = TRUE)
CIDtoCHEMBL
```

2. Get NSC Number for a list of CIDs

```{r eval=FALSE}

(annotation <- headers[Heading %like% 'NSC' & Type=="Compound", ]$Heading)

# get NSC Numbers for a list of CIDs
CIDtoNSC <- lapply(
  noDuplicatesNamesToCID$cids, 
  function(cid) as.data.table(getPubChemAnnotation(cid, annotationType=annotation)))

# merge results into a single data.table`
CIDtoNSC <- merge(rbindlist(CIDtoNSC, fill=TRUE), namesToCID, by.x = "cid", by.y = "cids", all.x = TRUE)
CIDtoNSC
```


3. use `getPubChemAnnotations` to get multiple annotations for a CID

```{r eval=FALSE}

annotations <- c('ChEMBL ID', 'NSC Number', 'Drug Induced Liver Injury', 'CAS', 'ATC Code')

CID <- noDuplicatesNamesToCID$cids[3]

CIDtoAnnotations <- getPubChemAnnotations(CID, annotations)
...
The `getPubChemAnnotation` function supports arbitrary annotation headers.
