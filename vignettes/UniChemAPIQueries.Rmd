---
title: "Querying UniChem with AnnotationG"
author:
...
**WARNING: The UniChem API has been updated to use POST requests and We
haven't had a chance to update our function yet. Please open an issue
at bhklab/AnnotationGx if you need these functions fixed!**

The UniChem database provides a publicly available RESTful web service API for
the retrieval of data in a programmatic fashion. All RESTful queries are
constructed using a base url. There are three types of input data for the
UniChem API; src_compound_id, src_id and InChIKey. src_id is the source which
is represented by an integer. src_compound_id is the compound represented by an
identifier for that specific source [1] . We can use the API to map between
external databases identifiers, take inchikey as a parameter and map to any
identifier, take in chemical id and source id and return the inchi and
inchikey structure and  takes in inchikey and names of databases and return the
...
`mapBetweenSources` function can be used to achieve this goal on a single
Chemical Id, however if we have a list of Chemical Ids for which we can use the
...
(result <- wIdentifierToInchiKey(src_compound_ids, target_names = src_id))
```
