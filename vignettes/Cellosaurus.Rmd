...
By default, the function will try to map using the common identifier name and will return the standard identifier
and the Cellosaurus accession number. The function also returns two additional columns `query` and `query:<from>` which
can be used to identify the original query and the original identifier type if needed. 
``` {r map one name each}
mapCell2Accession("hela", from = "id", numResults=1)
```
...
The main identifier for the A549 cell line is "A-549" with accesion id of "CVCL_0023" which does not appear in 
even the first 10 results (note: it would be the 40th result if we set `numResults=1000`) and while we could manually 
adjust every query to match the standard name, this is not feasible for hundreds of cell lines. 
...
This argument will use the parent cell line of each returned result and prioritize the results based on the accession id
...
``` {r map one name each}
mapCell2Accession("BT474", from = "id", numResults=1, prioritizeParent=TRUE)
```
